defmodule Datacop.Policy do
  ...
  def normalize_output(false, action), do: {:error, %Datacop.UnauthorizedError{action: action}}

  def normalize_output({:error, reason}, action),
    do: {:error, %Datacop.UnauthorizedError{message: to_string(reason), action: action}}
